...
- **プロファイルによるメモリコンテキストの分離**（個人、クライアント、企業）。解決順序は、明示的な `--profile`/`ENGRAM_PROFILE`、ワークスペースのデフォルト、ユーザーのデフォルトの順です。ワークスペースに固定されたプロファイルは、そのワークスペース内のすべてのCLI、MCP、およびエージェントフックのロードを制御します。
