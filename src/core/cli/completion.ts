...
export function completionScript(shell: 'bash' | 'zsh' | 'powershell' = 'bash'): string {
  ...
  return [
    ...
    '    resolve-conflicts|rc)',
      ...
      '      ;;',
    '    install-agent-hooks|iah)',
      '      if [[ $prev == install-agent-hooks || $prev == iah ]]; then COMPREPLY=( $(compgen -W "$agent_hook_targets --global --plan --force" -- "$cur") ); return; fi',
      '      COMPREPLY=( $(compgen -W "--global --plan --force" -- "$cur") )',
      '      return',
      '      ;;',
    '    uninstall-agent-hooks|uah)',
      '      if [[ $prev == uninstall-agent-hooks || $prev == uah ]]; then COMPREPLY=( $(compgen -W "$agent_hook_targets --global" -- "$cur") ); return; fi',
