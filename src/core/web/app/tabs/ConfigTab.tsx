...
export function ConfigTab({ data, reload, toast }: { data: PanelData; reload: () => Promise<void>; toast: ShowToast }) {
  ...
  return <>
    <SectionHeader title="Construct" copy="Settings applied across all workspaces." />
    ...
    <div className="grid-2">{Object.entries(grouped).reduce<any[][]>((cols, [group, groupFields], idx) => { cols[idx % 2].push(<Card key={group} title={group}>{groupFields.map((field) => <div key={field.key} className={'cfg-row' + (dirty[field.key] ? ' dirty' : '')} data-key={field.key}><div className="cfg-lbl">{field.label}{field.description ? <span className="cfg-desc">{field.description}</span> : null}{errors[field.key] ? <span className="cfg-error">{errors[field.key]}</span> : null}</div><div className="cfg-ctl">{renderControl(field)}{dirty[field.key] ? <button className="cfg-reset" onClick={() => resetField(field)}>Reset</button> : null}</div></div>)}</Card>); return cols; }, [[], []]).map((col, idx) => <div key={idx} className="grid-col">{col}</div>)}</div>
