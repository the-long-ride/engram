...
export function RuntimeTab({ data, toast }: { data: PanelData; toast: ShowToast }) {
  ...
  return <><SectionHeader title="Runtime" copy="Current Engram runtime and environment values." /><div className="rt-grid">{groups.map((group, idx) => <Card key={group.group || idx} title={group.group || 'Runtime'}>{(group.rows || []).map(([key, value]) => <div key={key} className="rt-row"><span className="rt-key"><span className="rt-key-pre">{String(key).split('.')[0]}</span>{String(key).includes('.') ? '.' + String(key).split('.').slice(1).join('.') : ''}</span><button className={'rt-val ' + valueClass(value) + (copiedKey === key ? ' copied-value' : '')} onClick={(event) => copyRuntimeValue(event, String(key), value)}>{copiedKey === key ? 'Copied' : String(value ?? '-')}</button></div>)}</Card>)}</div></>;
}
