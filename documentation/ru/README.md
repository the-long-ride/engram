...
- **Профили изолируют контексты памяти** (личный, клиенты, компания). Порядок разрешения следующий: явный `--profile`/`ENGRAM_PROFILE`, значение по умолчанию воркспейса, затем значение по умолчанию пользователя; закрепленный за воркспейсом профиль управляет всеми загрузками CLI, MCP и хуков агента в этом воркспейсе.
...
Для OpenCode генерируется такая запись MCP:
...
Локальный plugin остаётся в `~/.config/opencode/plugins/engram.js`; не добавляйте npm-стиль запись `plugin` для этого локального файла.
