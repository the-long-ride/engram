...
export async function getContext(cwd = process.cwd()): Promise<EngramContext> {
  const config = await loadConfig(cwd);
  const roots = scopeRoots(cwd);
  const ignore = await loadIgnore(cwd, config);
  const workspace = await exists(roots.workspace) ? await rebuildIndex(roots.workspace, 'workspace', ignore.patterns) : await loadIndex(roots.workspace);
  const global = await exists(roots.global) ? await rebuildIndex(roots.global, 'global', ignore.patterns) : await loadIndex(roots.global);
  const index = mergeIndexes(workspace, global);
  const hiddenCount = index.entries.filter((entry) => entry.ignored).length;
