...
```
v1 フックのインストールは `codex`、`claude`、`gemini`、`opencode` で利用可能です。Antigravity との互換性は現在 `gemini` を経由してルーティングされます。Cursor、Copilot、Cline、Windsurf/Cascade は、フックのインターフェースがプロンプト実行時の信頼性の高いコンテキスト注入をサポートするまで、指示/スキルセット/手動ロード駆動のままとなります。
...
| **エージェントフック導入** | `engram link codex|claude|gemini|opencode` | 一度だけターミナルで実行 |
