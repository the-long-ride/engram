...
- Write/change commands (\`save/save-session/observe --propose/take-control/import/archive/resolve-conflicts/install-hooks/install-skillset/sync/init --submodule|--global-remote\`): state the change first, then show Engram result.
...
- Prefer MCP for \`load\`, \`search\`, \`verify\`, \`status\`, \`save\`, and \`engram_autosave\` proposals when available.
...
- If the request is \`/engram ss -a\` or legacy \`/engram at -a\`, treat it as \`/engram save-session --accept-all\`; \`-a\` is the human's explicit accept-all approval for this shortcut.
- If the request is \`/engram save-session --accept-all\` or legacy \`/engram autosave --accept-all\`, treat the flag as explicit approval: generate concise \`TYPE: ... | TEXT: ...\` candidates when needed, pass them to the CLI with \`--accept-all\`, and report saved files.
