...
```
v1 훅 설치는 `codex`, `claude`, `gemini`, `opencode`에서 사용할 수 있습니다. Antigravity 호환성은 현재 `gemini`를 통해 라우팅됩니다. Cursor, Copilot, Cline, Windsurf/Cascade는 훅 표면이 프롬프트 시점에 안정적인 컨텍스트 주입을 지원할 때까지 지침/스킬셋/수동 로드 방식으로 유지됩니다.
...
| **에이전트 훅 설치** | `engram link codex|claude|gemini|opencode` | 터미널에서 한 번 실행 |
