...
- Write/change commands (\`save/save-session/observe --propose/take-control/import/archive/clone-memory/resolve-conflicts/install-hooks/install-skillset/upgrade/sync/init --submodule|--global-remote\`): state the change first, then show Engram result.
...
- Never add \`--accept-all\` yourself. Otherwise never bypass Engram's approval gate. For writes, remotes, conflicts, hooks, import, clone-memory, sync, or adapter install, state the change first and show the short result.
