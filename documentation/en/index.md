...
- `save-session` / `ss` for several memories from a session, with optional `--query-level <n>` to mine up to n recent accessible chats; `/engram ss -f last 50 sessions` normalizes to `engram save-session --query-level 50 --force`
