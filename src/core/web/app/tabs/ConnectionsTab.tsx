...
export function ConnectionsTab({ active, toast }: { active: boolean; toast: ShowToast }) {
  ...
  return <><SectionHeader title="Connections" copy="Connect Engram to local AI agents." helpHref={entryDoc('connections')} actions={<Button onClick={scanAgents}>{loading ? 'Scanning...' : 'Refresh'}</Button>} />
