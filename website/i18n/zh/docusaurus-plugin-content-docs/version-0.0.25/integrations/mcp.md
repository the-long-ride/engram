...
MCP 主机应将 `engram_save` 和 `engram_autosave` 视为**仅提案**工具；它们仍必须通过人眼可见的 CLI 审批流路由最终写入。`engram_load` 默认使用 `--for-agents`（可通过 `forAgents: false` 选择关闭）。

## 接受全部规则

显式的 `/engram save-session --accept-all` 请求（包括快捷方式 `/engram ss -a`）应使用 CLI 写入路径，因为 MCP 自动保存仍然是仅提案的。计数的快捷方式 `/engram ss -a last 50 sessions` 应使用 `engram save-session --query-level 50 --accept-all`。
