...
test('upgrade --latest refreshes registered global opencode paths and plugin', async () => {
  ...
  await writeFile(path.join(configHome, 'opencode', 'plugins', 'engram.js'), '// Generated by Engram OpenCode hook installer. Edit with care.\nexport const Old = async () => ({})\n');

  const upgraded = await runEngram(cwd, globalEnv, ['upgrade', '--latest', '--no-version-check']);
  assert.equal(upgraded.code, 0, upgraded.stderr);
  assert.match(upgraded.stdout, /UPDATED global agent hooks/);

  const agents = await readFile(path.join(configHome, 'opencode', 'AGENTS.md'), 'utf8');
  assert.match(agents, /Full guide: `~\/\.config\/opencode\/engram\.md`/);
  assert.match(await readFile(path.join(configHome, 'opencode', 'skills', 'engram', 'SKILL.md'), 'utf8'), /Engram Memory Management Skill/);
  assert.match(await readFile(path.join(configHome, 'opencode', 'plugins', 'engram.js'), 'utf8'), /EngramOpenCodePlugin/);

  const config = JSON.parse(await readFile(path.join(configHome, 'opencode', 'opencode.json'), 'utf8'));
  assert.equal(config.mcp.engram.command[0], 'npx');
  assert.ok(!config.instructions?.includes('.opencode/engram.md'));
  await rm(cwd, { recursive: true, force: true });
});
...
test('upgrade --latest refreshes registered global opencode.jsonc paths and plugin', async () => {
  ...
  await writeFile(path.join(configHome, 'opencode', 'opencode.jsonc'), [
    '{',
    '  "$schema": "https://opencode.ai/config.json",',
    '  "instructions": [".opencode/engram.md"],',
    '  "plugin": ["user-plugin"],',
  ...
  await rm(path.join(configHome, 'opencode', 'opencode.json'), { force: true });
  await writeFile(path.join(configHome, 'opencode', 'plugins', 'engram.js'), '// Generated by Engram OpenCode hook installer. Edit with care.\nexport const Old = async () => ({})\n');

  const upgraded = await runEngram(cwd, globalEnv, ['upgrade', '--latest', '--no-version-check']);
  assert.equal(upgraded.code, 0, upgraded.stderr);

  const config = JSON.parse(await readFile(path.join(configHome, 'opencode', 'opencode.jsonc'), 'utf8'));
  assert.deepEqual(config.plugin, ['user-plugin']);
  assert.equal(config.mcp.engram.command[0], 'npx');
  assert.ok(!config.instructions?.includes('.opencode/engram.md'));
  await assert.rejects(readFile(path.join(configHome, 'opencode', 'opencode.json'), 'utf8'));
  await rm(cwd, { recursive: true, force: true });
});
