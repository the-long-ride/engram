...
- **配置文件 (Profiles) 隔离内存上下文**（个人、客户和企业）。解析顺序为显式指定 `--profile`/`ENGRAM_PROFILE`、工作区默认值、然后是用户默认值；固定到工作区的配置文件将控制该工作区中的所有 CLI、MCP 和智能体钩子加载。
...
| **安装智能体 Hooks** | `engram link codex|claude|gemini|opencode|cursor|windsurf` | 在终端执行一次 |
