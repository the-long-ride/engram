...
export const HELP_DATA: HelpSection[] = [
  ...
  },
  {
    ...
    commands: [
      ...
      { command: 'engram save-session [--file transcript.md] [--scope workspace|global|both] [--role role] [--query-level n] [--force] [--inbox] [--show-rule-variants] [session-summary]', alias: 'ss', purpose: 'Propose multiple memories; force writes ready candidates, defers related-memory decisions with ID-only inspect guidance, and --inbox writes resumable receipt files under .agents/.engram/inbox/' },
      ...
      { command: 'engram load [--all] [--budget-tokens n] [--dry-run] [--explain] [--full] [--json] [query]', alias: 'ld', purpose: 'Route compact agent memory by default; --budget-tokens caps rendered compact context, --explain shows ranking signals, and --json emits a versioned contract envelope' },
      ...
      { command: 'engram doctor [workspace|global|all] [--strict] [--json]', purpose: 'Run composed diagnostics for config, roots, hashes, invalid files, index, graph, policy, and host linkage' },
      { command: 'engram policy init|show|validate|audit|rollback <audit-id> [--strict] [--json]', purpose: 'Inspect policy, view ID-only audit receipts, validate CI configuration, or roll back a policy-approved write' },
      { command: 'engram autosave --policy [--dry-run] [--json] [--scope workspace|global] <candidate>', purpose: 'Evaluate one candidate against explicit local policy before any autonomous write' },
      { command: 'engram capabilities [host] [--json]', purpose: 'Show declared host adapter capabilities and unsupported integration surfaces' },
  ...
  },
  {
    ...
    commands: [
      ...
      { command: 'engram review list|inspect|dismiss|verify|supersede|archive|inbox|apply|cleanup [--json] [--kind <kind>] [--note text] [--reason text] [--depends-on id] [--update id] [--force] [<receipt-id|memory-id>]', purpose: 'Review derived findings and apply deferred-save inbox receipts; relation overrides use IDs only and never expose memory bodies or authors' },
      { command: 'engram benchmark <cases.json> [--json] [--baseline report.json] [--write-report path] [--fail-on forbidden,recall,...]', alias: 'bm', purpose: 'Run versioned retrieval regression metrics without exposing memory bodies' },
