...
- Read commands (\`load/search/graph/benchmark/verify/health/stats/audit/dry-run/entry\`): one-line summary. Use \`--all\` only when broad context is requested.
- Write/change commands (\`save/autosave/observe --propose/take-control/import/archive/resolve-conflicts/install-hooks/install-skillset/sync/init --submodule|--global-remote\`): state the change first, then show Engram result.
...
- For raw notes, use \`engram observe\` to create sanitized inbox files; only \`observe --propose\` or \`autosave --file\` can convert them into approved memory.
- Use \`engram graph\` or \`engram quality-check\` to detect contradiction candidates; use \`engram archive --reason <why> <id|file>\` to remove wrong memory from active routing after approval.
...
- If the request is \`/engram observe\`, save the sanitized inbox note first; use \`--propose\` only when the human wants Engram to mine it through autosave.
