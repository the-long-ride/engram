...
아닙니다. 에이전트가 후보를 제안하면 사람이 승인합니다. 직접 터미널 CLI는 A/B/C를 사용합니다. AI 에이전트 채팅은 `yes`/`audit`/`cancel`을 사용합니다. 명시적인 전체 승인 요청(`ss -f`)만 모든 후보를 저장하며, 에이전트는 사용자가 요청하지 않는 한 `--force`을 추가해서는 안 됩니다.
