...
export const HELP_DATA: HelpSection[] = [
  {
    ...
    commands: [
      ...
      { command: 'engram init [--force] [--no-skillset] [--skillset target] [--submodule] [--submodule-remote <git-url>] [--no-global] [--global-path path] [--global-remote <git-url>] [--global-branch main]', alias: 'i', purpose: 'Initialize or reconcile memory and install compact agent skillset instructions by default' },
...
export function completionScript(shell: 'bash' | 'zsh' | 'powershell' = 'bash'): string {
  ...
  if (shell === 'zsh') {
    return [
      ...
      '  case $words[2] in',
      '    init|i)',
      '      _arguments "--force" "--no-skillset" "--skillset=[target]" "--submodule" "--submodule-remote=[git-url]" "--no-global" "--global-path=[path]" "--global-remote=[git-url]" "--global-branch=[branch]"',
      '      ;;',
      '    completion|c)',
  ...
  return [
    ...
    '  case "${words[1]}" in',
    '    init|i)',
    '      COMPREPLY=( $(compgen -W "--force --no-skillset --skillset --submodule --submodule-remote --no-global --global-path --global-remote --global-branch" -- "$cur") )',
    '      return',
    '      ;;',
    '    completion|c)',
