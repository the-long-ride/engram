...
export function planArchive(ctx: EngramContext, target: string, reason = ''): ArchivePlan {
  ...
  const entries = ctx.index.entries.filter((entry) => {
    if (wanted.scope && entry.scope !== wanted.scope) return false;
    return entry.id === wanted.value || entry.file === wanted.value || entry.file.endsWith(`/${wanted.value}`);
  });
...
function normalizeTarget(target: string): { scope?: Scope; value: string } {
