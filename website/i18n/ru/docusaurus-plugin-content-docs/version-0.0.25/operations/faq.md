...
Нет. Агенты предлагают кандидатов; люди одобряют. Прямой интерфейс CLI в терминале использует A/B/C. Чат ИИ-агента использует `yes`/`audit`/`cancel`. Только явные запросы на принятие всего (`ss -a`) сохраняют каждого кандидата, и агенты не должны добавлять флаг `--accept-all`, если только человек не запросил его сам.
