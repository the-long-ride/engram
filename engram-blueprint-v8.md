...
RULE 11 — ROUTE `/engram` slash requests through Engram itself.
          Treat `/engram <args>` as `engram <args>` or the matching MCP tool.
          Never implement a separate hidden write path inside the agent.
```
...
│      └─ If command is `engram save knowledge` with no text:
│         agent summarizes durable knowledge from its current work first.
│
...
| `engram: save knowledge [text]` | Save knowledge; if text is omitted, ask the agent to generate a concise summary of durable knowledge from its current work, then run the normal A/B/C approval UI |
...
Generated skillset adapters also expose `/engram <args>` for agents that support
project slash commands or Agent Skills. The agent must route the request to the
same Engram CLI or MCP flow the human could inspect directly:

| Slash request | Route |
|---|---|
| `/engram load <query>` | `engram_load` MCP tool or `engram load <query>` |
| `/engram save knowledge` | generated knowledge proposal, then approval gate |
| `/engram verify` | `engram_verify` MCP tool or `engram verify` |
| `/engram resolve-conflicts --dry-run` | CLI preview only |
| `/engram install-skillset slash` | install slash adapters |

Supported generated adapters: `.claude/skills/engram/SKILL.md`,
`.cursor/commands/engram.md`, `.gemini/commands/engram.toml`, and
...
```
/engram <command>      ← slash-command bridge: route to Engram CLI/MCP
                          /engram init, /engram load, /engram save knowledge

engram: <command>      ← memory operations: work with memory content
                          engram: save, engram: on/off, engram ignore: ...
```

In current implementation, `/engram <args>` is the universal agent-facing
surface. The agent treats it as `engram <args>` and still applies the same
approval, ignore, hash, and conflict safeguards. Legacy `engram:` phrasing
remains valid conversational shorthand.
...
### Pattern G — Universal Skillset Slash Adapter

`engram install-skillset all` writes agent-host instruction files plus a slash
adapter set. `engram install-skillset slash` writes only slash-capable adapters.

```
.claude/skills/engram/SKILL.md       → /engram in Claude Code
.cursor/commands/engram.md           → /engram in Cursor
.gemini/commands/engram.toml         → /engram in Gemini CLI
.agents/skills/engram/SKILL.md       → Agent Skill hosts, including Codex/Antigravity
```

The slash adapter is a router, not a writer. It passes `/engram <args>` to the
Engram CLI or to MCP tools for read/proposal commands. All writes still pass
through the same A/B/C approval gate, sensitive-data guard, injection guard, and
hash update path.
...
Phase 5 — Agent Ecosystem  ← v0.7
  ...
  ✓ Universal skillset installer — AGENTS.md, Copilot, Claude, Cursor, Gemini, Cline, Windsurf, OpenCode, MCP
  ✓ Slash command adapters — `/engram <args>` routed through CLI/MCP
...
| 18 | `/engram` vs `engram:` syntax | `/engram <args>` is the universal agent-facing command surface and routes to `engram <args>` or MCP. `engram:` remains conversational shorthand for memory operations. |
...
| 25 | Slash command adapters | Generated slash files are routers only. They cover the full CLI command surface while preserving approval, ignore, hash, and conflict boundaries. |
...
### 24.4 Universal Skillset & Slash Commands

`engram install-skillset` generates host adapters without overwriting
human-authored files. `all` installs instruction files, MCP config, and the
slash adapter set. `slash` installs only native `/engram` prompt adapters for
hosts that support project slash commands.

Supported target classes:
- instruction files: `AGENTS.md`, `CLAUDE.md`, `GEMINI.md`, Copilot,
  Cursor rules, Cline, Windsurf, and OpenCode
- Agent Skills: `.agents/skills/engram/SKILL.md` and
  `.claude/skills/engram/SKILL.md`
- slash commands: `.cursor/commands/engram.md` and
  `.gemini/commands/engram.toml`
- MCP config: `.mcp.json`

The adapter contract is intentionally thin: `/engram <args>` becomes
`engram <args>` or a matching MCP read/proposal call. This keeps every agent on
one command surface and one set of safety guarantees.
