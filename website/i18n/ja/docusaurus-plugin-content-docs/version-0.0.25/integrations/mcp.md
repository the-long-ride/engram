...
MCP ホストは、`engram_save` および `engram_autosave` を**提案専用**ツールとして扱う必要があります。最終的な書き込みは、人間が確認できる CLI 承認ワークフローを介してルーティングされる必要があります。`engram_load` のデフォルトは `--for-agents` です（`forAgents: false` でオプトアウト可能）。
