...
| 전술적 과제 | Engram의 해결책 |
| --- | --- |
| **너무 많은 규칙으로 인한 컨텍스트 비대화** | 현재 작업에 매칭되는 지식만 검색해 기본적으로 최대 8개의 파일로 팩을 구성해 제공합니다. |
...
넓은 범위의 검색으로 인해 매칭되는 메모리가 8개를 초과하는 경우, `engram load`는 상위 8개를 로드하기 전에 태그, 타입, 최근 순서, 그래프 및 sqlite-vec 벡터 점수를 기반으로 가중치를 재조정합니다. 에이전트가 `engram load --dry-run "<작업>"`을 사용하여 후보들의 수와 필터링 추천 태그를 미리보거나, 넓은 범위의 로드를 의도할 때는 `--all`을 지정할 수 있습니다.
