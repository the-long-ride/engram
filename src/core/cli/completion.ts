...
export function completionScript(shell: 'bash' | 'zsh' | 'powershell' = 'bash'): string {
  ...
  if (shell === 'powershell') {
    return [
      ...
      ')',
      '$engramInjectArgs = @(\'--force\', \'--global-only\', \'--scope\', \'--no-skillset\', \'--skillset\', \'--submodule\', \'--submodule-remote\', \'--no-global\', \'--global-path\', \'--global-remote\', \'--global-branch\')',
      ...
      '        { $_ -in @(\'load\', \'ld\') } { @(\'--all\', \'--budget-tokens\', \'--dry-run\', \'-f\', \'--full\'); break }',
  ...
  return [
    ...
    '      ;;',
    '    load|ld)',
    '      # --all: bypass compact cap (broad context), --budget-tokens: compact payload cap, --dry-run: preview without printing, -f/--full: broader legacy output',
    '      COMPREPLY=( $(compgen -W "--all --budget-tokens --dry-run -f --full" -- "$cur") )',
    '      return',
    '      ;;',
    '    graph|g)',
