...
```text
/engram load --for-agents "текущая задача"
```

Или из терминала:

```bash
engram load --for-agents "<задача>"
```
...
`-a` означает, что человек явно одобряет каждого рекомендованного агентом кандидата. Агенты не должны добавлять `--accept-all`, если человек этого не просил.
