...
| --- | --- |
| **Слишком много правил раздувают контекст** | Направляет и сокращает соответствующую задаче память в компактный пакет контекста, по умолчанию 8 записей. |
...
Когда широкие запросы соответствуют большему числу записей памяти, чем разрешает настроенный лимит загрузки, `engram load` повторно ранжирует их с помощью тегов, типа, давности, графа и опциональных векторных сигналов sqlite-vec перед загрузкой компактного пакета. Обычная загрузка показывает выбранное количество и общий связанный счетчик, например `loaded 8 memory files / 14 total related memories`. Используйте `engram load --dry-run "<задача>"` для предварительного просмотра связанных счетчиков и тегов, `engram set-load-limit 1..32` для настройки лимита или `--all`, когда широкий контекст является намеренным.

Воспоминания также могут объявлять зависимости через `depends_on` и необязательный уровень вроде `level: advanced`. Граф выстраивает их от базовых правил к более глубоким знаниям, а `engram load` сохраняет нужные предварительные воспоминания рядом с зависимым воспоминанием в компактном пакете. Во время `engram save` предварительный просмотр показывает связанные существующие воспоминания или возможные дубликаты, чтобы их можно было реструктурировать до сохранения.
...
- **Клонировать память workspace/global:** `engram clone-memory workspace global` / `engram clone-memory global workspace --force` (Агент: `/engram clone workspace memory to global`) (`--metacognize` routes cloned memories through save-session-style approval instead of raw copy.)
...
| **Клонировать память Workspace/Global** | `engram clone-memory workspace global` / `engram clone-memory workspace global --metacognize` | `/engram clone workspace memory to global` |
